...
{
    public class SwapRequestIncidentIBK : GenericRequest
    {
        ...
        IncidentNP incidentNP = new IncidentNP();
        ...
        public override async Task CollectDataAsync()
        {
            ...
            incidentNP.AffectedCI = cabinet.Name;
            incidentNP.Description = new string[] { $"Item {cabinetAction.PutItemDescription} met CI-nummer {putItem.ExternalReferenceID} van het type {putItem.ItemType.Description} is op {cabinetAction.ActionDT.ToString("dd-MM-yyyy")}, {cabinetAction.ActionDT.ToString("HH:mm")} defect gemeld door gebruiker {cabinetAction.CTAMUserName} met errorcode: {errorCode.Code} {cabinetAction.ErrorCodeDescription}. Gebruiker {cabinetAction.CTAMUserName} heeft nu vervangend item {cabinetAction.TakeItemDescription} met CI-nummer {takeItem.ExternalReferenceID} van het type {takeItem.ItemType.Description} in gebruik. Het defecte item kan worden uitgenomen bij IBK {cabinet.Name} op locatie {cabinet.LocationDescr} en een werkend item van het type {takeItem.ItemType.Description} dient te worden ingelegd." };
            incidentNP.Location = cabinet.LocationDescr;
            incidentNP.Title = $"IBK {cabinetAction.CabinetName}: Uithalen en Inleggen {putItem.ItemType.Description}";
        }
