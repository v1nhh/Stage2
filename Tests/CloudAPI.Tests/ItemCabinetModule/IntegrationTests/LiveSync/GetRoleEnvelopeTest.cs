...
{
    public class GetRoleEnvelopeTest : AbstractIntegrationTests
    {
        ...
        public GetRoleEnvelopeTest() : base("CTAM_GetRoleEnvelope")
        {
        ...
        public async Task TestGetRoleLiveSyncEnvelope(int roleID, bool roleFound,
        ...
        {
            // Arrange
            var query = new GetCollectedEnvelopeQuery(new CollectedLiveSyncRequest() { RoleIDsForEnvelope = { roleID } });
            ...
            using (var context = new MainDbContext(ContextOptions, null))
            {
                ...
                // Assert
                Assert.Equal(roleFound, envelope.Roles.Any(r => r.ID == roleID));
        ...
        public static readonly object[][] RoleWrongData =
        {
            new object[] { ROLE_WITHOUT_CABINET_ID, false, false },
            new object[] { ROLE_WITH_ITEM_TYPE_WITHOUT_CABINET_STOCK_ID, true, false },
        };

        [Theory, MemberData(nameof(RoleWrongData))]
        public async Task TestGetWrongRoleLiveSyncEnvelope(int roleID, bool roleFound, bool cabinetStockFound)
        {
            // Arrange
            var query = new GetCollectedEnvelopeQuery(new CollectedLiveSyncRequest() { RoleIDsForEnvelope = { roleID } });
            var mapper = new MapperConfiguration(c =>
            ...
            using (var context = new MainDbContext(ContextOptions, null))
            {
                ...
                // Assert
                Assert.Equal(roleFound, envelope.Roles.Any(r => r.ID == roleID));
                Assert.Equal(cabinetStockFound, envelope.CabinetStocks.Count > 0);
            }
        }
    }
}
